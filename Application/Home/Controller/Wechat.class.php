	...
	public function getRev()
	{
		...
		if (!empty($postStr)) {
			$this->_receive = (array)simplexml_load_string($postStr, 'SimpleXMLElement', LIBXML_NOCDATA);
                        ///data/store1/upload/
                        //file_put_contents('/data/store1/upload/'.time().'.txt', json_encode($this->_receive));
		}
		return $this;
	}
	...
	*/
	...
	public function text($text='')
	{
        ...
        public function transCS(){
            ...
            echo sprintf($xmlTpl, $this->getRevFrom(), $this->getRevTo(), time());
        }
	/**
	 * 设置回复消息
...
<Nonce><![CDATA[%s]]></Nonce>
	 ...
	 */
	 /*
	protected function setCache($accesstoken){
            $data = array(
                'expire_time'=>time() + 7000,
                'access_token'=>$accesstoken
            );
            $redis = new Redis();
            $redis->connect('10.10.68.171','6379');
            $redis->hMset('cache:token',$data);
	}
	*/
	/*
	protected function setCacheTicket($ticket){
            $data = array(
                'expire_time'=>time() + 7000,
                'jsapi_ticket'=>$ticket
            );
	...
	*/
	...
	protected function getCache(){
		//TODO: get cache implementation
                $redis = new Redis();
                $redis->connect('10.10.68.171','6379');
                $data = $redis->hGetAll("cache:token");
	...
	*/
	/*
	protected function getCacheTicket(){
		//TODO: get cache implementation
                $redis = new Redis();
                $redis->connect('10.10.68.171','6379');
                $data = $redis->hGetAll("cache:ticket");
                if ($data['expire_time'] > time()) {
	...
	public function checkAuth($appid='',$appsecret='',$token=''){
		...
		if(isset($_SESSION['access_token'])){
			if ($_SESSION['access_token']['expire_time'] > time()) {
		...
		if ($result)
		{
			...
			if(isset($_SESSION['access_token'])){
				...
				$_token['expire_time'] = time() + $_token['expire_time'];
	...
	public function getJsTicket($appid='',$jsapi_ticket=''){
                ...
                if ($result)
		{
			...
			//$this->setCacheTicket($this->jsapi_ticket);
			return $this->jsapi_ticket;
		}
		return false;
	}
	...
	public function uploadMedia($data, $type){
		...
		$result = $this->http_post_form(self::API_URL_PREFIX.self::MEDIA_UPLOAD_URL.'access_token='.$this->access_token.'&type='.$type,$data);
                if ($result)
		{
			$json = json_decode($result,true);
			if (!$json || !empty($json['errcode'])) {
         ...
         * 
         */
        public function http_post_form($url,$param){
                $oCurl = curl_init();
                if(stripos($url,"https://")!==FALSE){
                        curl_setopt($oCurl, CURLOPT_SSL_VERIFYPEER, FALSE);
                        curl_setopt($oCurl, CURLOPT_SSL_VERIFYHOST, false);
	...
	public function sendGroupMassMessage($data){
		...
		{      
			...
			if (!$json || !empty($json['errcode'])) {
				$this->errCode = $json['errcode'];
				$this->errMsg = $json['errmsg'];
				return array('code'=>  $this->errCode,'msg'=>  $this->errMsg);
			}
			return false;
		}
		return false;
